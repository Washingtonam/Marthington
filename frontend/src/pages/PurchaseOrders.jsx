  ...
  return (
    <section className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
              Track incoming orders, approvals, and supplier commitments in one place.
            </p>
          </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {[
          { title: "Pending approvals", value: "4", icon: FiClipboard },
          { title: "Completed this month", value: "12", icon: FiCheckCircle },
        ].map((card) => {
      ...
      <div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
        <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-6 text-center text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-400">
          Purchase order workflows and approval history will be surfaced here as the procurement module is completed.
        </div>
      </div>
    </section>
  );
